...
**Architecture:** Keep the existing Admin Config controller shape and reuse `EtcdStagedRecoveryStore`, the same backend used by `/staged-recovery/start-limit`. Runtime-only drafts apply through `runtime_etcd`; drafts containing reconfigure-required changes are rejected because `masakari.conf` values are immutable through Masakari API.
...
- [x] Add a test that applies a mixed draft with `max_parallel_starts_per_host` and `batch_delay`, expects `409 Conflict`, and asserts `set_max_parallel_starts_per_host` was not called.
...
- [x] Reject mixed or reconfigure-required drafts before creating an apply job.
